...
@NoArgsConstructor
@AllArgsConstructor
@ToString(exclude = {"file", "comment"}) // 양방향
@Builder
@Entity                  // 엔티티 객체 정의
...
public class Article {
     ...
     */
    @ManyToOne(fetch = FetchType.LAZY) // 하나의 user는 여러개의 article을 가질 수 있음(article을 기준으로) fetch 생략 가능
    ...
    /*
        @OneToMany
         ...
         - Article 엔티티와 File 엔티티간의 관계 방향성을 고려해서 연관관계 설정
        ...
        @Transactional
         ...
         - 하나의 SELECT는 한번의 세션처리로 트랜잭션으로 처리하지 않으면 no session에러 발생
     ...
     */
    @OneToMany(fetch = FetchType.LAZY, mappedBy = "article") // mappedBy 필수로 써야 함
