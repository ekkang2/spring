...
public class SecurityConfig {
    ...
    public SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(authorize -> authorize
                                                    ...
                                                    .requestMatchers("/admin/**").hasRole("ADMIN") // 관리자페이지는 관리자만 접근가능
                                                    .requestMatchers("/manager/**").hasAnyRole("ADMIN", "MANAGER") // 매니저페이지는 관리자, 매니저만 접근가능
                                                    .requestMatchers("/staff/**").hasAnyRole("ADMIN", "MANAGER", "STAFF") // 스태프페이지는 관리자, 매니저, 스태프 모두 접근가능
                                                    .anyRequest().permitAll()); // 그밖에 모든 요청에는 접근을 허용
